  ...
  applyFilter(filter) {
    ...
    if (filter === 'GEN') {
      return this.visibleClasses = this.classes.filter(c =>
        !c.club.clubName.startsWith('CH') &&
        !c.club.clubName.startsWith('PO') &&
        !c.club.clubName.startsWith('SP'));
    }

    return this.visibleClasses = this.classes.filter(c => c.club.clubName.startsWith(filter));
  }
}
